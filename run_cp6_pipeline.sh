...
for S in "${SEEDS[@]}"; do
  echo "########## CP6 SEED $S START $(date +%H:%M:%S) ##########"
  ...
  if have "$DENSE/run_*/best_checkpoint.pth"; then echo "[skip] dense B0 seed $S"; else
    ...
    ( cd src && python -m training.efficientnetb0 --seed "$S" --dataset_name "$DS" )
  fi
  ...
  echo "########## CP6 SEED $S DONE $(date +%H:%M:%S) ##########"
done
echo "########## CP6 PIPELINE ALL DONE ##########"
