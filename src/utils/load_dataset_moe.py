...
This module provides a PyTorch Dataset implementation for loading plant disease
images with stratified train/validation/test splitting capabilities. It supports
custom image transformations and optional context feature extraction.
...
class LoadDataset(Dataset):
    ...
    Attributes:
        ...
        return_context (bool): Flag to include context features in dataset samples.
        context_extractor (Optional[Callable]): Function to extract contextual features from images.
    ...
    def __init__(
        ...
        Args:
            ...
            return_context (bool, optional): Whether to include context features in
                returned samples. If True, context_extractor must be provided.
                Defaults to False.
            context_extractor (Callable, optional): Function that takes an image array
                and returns context features as numpy array or tensor. Called when
                return_context=True. Defaults to None.
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int, Optional[torch.Tensor]]:
        ...
        Loads an image file from disk, applies optional transformations, computes
        optional context features, and returns a tuple of tensors and labels.

        Image Loading and Processing:
            1. Open image from disk and convert to RGB
            2. Convert to numpy array
            3. Apply transformations if transform pipeline provided
            4. Extract context features if return_context=True and context_extractor provided
            5. Return tensors in the specified format
        ...
        Returns:
            Tuple[torch.Tensor, int, Optional[torch.Tensor]]:
                image (torch.Tensor or np.ndarray): Transformed image array/tensor.
                ...
                context_features (torch.Tensor or bool): Context features extracted from
                    the image if return_context=True, otherwise False. Features are
                    torch.Tensor of dtype float32.
        ...
        Example:
            ...
            >>> print(context)  # False or torch.Tensor
        """
