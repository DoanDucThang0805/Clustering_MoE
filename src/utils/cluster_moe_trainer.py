...
class ClusterMoETrainer:
    ...
    def _monitor_expert_usage(
        ...
        expert_mask  = torch.nn.functional.one_hot(top_indices, num_classes=N).float()
        ...
        class_count   = expert_mask.T @ label_onehot              # (N, C)
    ...
    def train(self) -> None:
        ...
        for epoch in tqdm.tqdm(range(self.num_epochs), desc="Epochs"):
            ...
            for images, labels in self.train_loader:
                ...
                logits, weights, top_indices, scores = self.model(images)
            ...
            if validation_acc > best_val_acc + self.val_acc_threshold:
                logger.info(
                    f"Val acc improved ({best_val_acc:.4f} → {validation_acc:.4f})"
                )
    ...
    def _save_plots(self) -> None:
        ...
        plt.title("Learning Rate"); plt.xlabel("Epoch"); plt.ylabel("LR"); plt.yscale("log")

        plt.tight_layout()
        plt.savefig(os.path.join(self.run_dir, "loss_acc_plot.png"))
        plt.close()

        # ── Expert usage bar ─────────────────────
        plt.figure()
        plt.bar(range(self.model.num_experts), self.expert_sample_count.numpy())
        plt.title("Expert Usage"); plt.xlabel("Expert ID"); plt.ylabel("Sample Count")
        plt.savefig(os.path.join(self.run_dir, "expert_usage.png"))
        plt.close()

        # ── Expert-class heatmap ──────────────────
        heatmap      = self.expert_class_count.numpy()
        heatmap_norm = heatmap / (heatmap.sum(axis=1, keepdims=True) + 1e-9)

        plt.figure(figsize=(10, 6))
        im = plt.imshow(heatmap_norm, cmap="Blues")
        plt.colorbar(im)
        ...
        logger.info(f"Saved plots to {self.run_dir}")
